...
class DES_Cipher_Encryption:
    ...
    def encrypt(self):
        ...
        final_cipher_8_bit = list_of_lists = [final_cipher_bit[i:i+8] for i in range(0, len(final_cipher_bit), 8)]
...
# ask for input and key seperately
    def des_encryption(self):
        func = Functions()
        plaintext = self.getmsg_des()
        # ask for different modes of encryption 
        des_encrypt = DES_Cipher_Encryption(p_input=plaintext)
        des_encrypt.key = self.getmsg_des()
        cipher_t = des_encrypt.encrypt()
    
    def getmsg_des(self):
        func = Functions()
        msg = input("Provide the msg for encryption with a string or strings that matches a total of 64-bit length: ")
        print("\n")
        while not msg or not func.is_valid_input(msg):
    ...
    def mode_operation(self,word):
        print("Given the provided block of text is bogger than the base amount, which mode of encryption would you prefer to be used? ")
